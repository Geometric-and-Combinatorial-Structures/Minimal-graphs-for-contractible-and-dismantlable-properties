/*
This program is to collect cadinalities of acyclicc grpah and check misings runs
*/
	...
	for(V=1;V<=10;V++){
		...
		for(E=V-1;E<=V*(V-1)/2;E++){	
			...
			if(IFc==NULL){
				...
				printf("there are missing v=%d e=%d sic file",V,E);
				getchar(); return 1;
			}
			fscanf(IFc,"%ld",&card); fclose(IFc);
			g+=card;
			fprintf(OFve,"%d %d %ld\n",V,E,card);
		}
		ga+=g;
	...
	for(E=17;E<=39;E++){
		...
		for(M=1;M<=nM;M++){
			...
			if(IFc!=NULL){
				...
				printf("\nProblems with input file %s  \n",NameIn_c); 
				printf("there are missing v=%d e=%d M=%d   sic file",V,E,M);
				getchar();return 1;
			}
		}
		fprintf(OFve,"%d %d %ld\n",V,E,g);
