/*
  ...
  and save in a file which have trivial homology ( aciclic graphs) 
  ...
  Uses: 
     ...
     acyclic V E n1			: graphs with V vertices, E edges and n1_th million  will be proceced (by big files) 
     acyclic V E n1 n2			: graphs with V vertices, E edges and n1_th to n1_th ( millions  will be proceced  )
...
public:
	...
	float compute_dim_0_pairs(std::vector<diameter_index_t>& edges, std::vector<diameter_index_t>& columns_to_reduce) {
	...
	float compute_pairs(const std::vector<diameter_index_t>& columns_to_reduce, entry_hash_map& pivot_column_index, const index_t dim) {
	...
	float * compute_barcodes() {

		int nDims = dim_max + 1;
		float * resultado = new float[nDims];
		
		std::vector<diameter_index_t> simplices, columns_to_reduce;

		*resultado = compute_dim_0_pairs(simplices, columns_to_reduce);

		for (index_t dim = 1; dim <= dim_max; ++dim) {
			entry_hash_map pivot_column_index;
			pivot_column_index.reserve(columns_to_reduce.size());

			*(resultado + (int) dim) = compute_pairs(columns_to_reduce, pivot_column_index, dim);
...
double * GenerateRandomGraph(int numVertices){
    //Seed for random numbers generation
    srand((unsigned int)time(NULL));
    
    float randomDistance; //Stores the random distance (0 - 1000)
    long pos; //Stores the file length
    std::ofstream file; //file object

	double total_entries = ( (numVertices - 1) * (numVertices) ) / 2;
	double * graph = new double[(int)total_entries];
	for(int i = 0 ; i < total_entries ; i++){
		*(graph + i) = static_cast <float> (rand()) / (static_cast <float> (RAND_MAX/1000));
	}
...
compressed_lower_distance_matrix read_graph( double  * graph, double num_vertices) {
	std::vector<value_t> distances;
	value_t value;
	int total_entries = ( (num_vertices - 1) * num_vertices) / 2; 
	for (int i = 0; i < total_entries; ++i) {
			value = *(graph + i);
			distances.push_back(value);
	}
...
void Homology( double * graph, double num_entries, index_t dimMax, value_t Threshold, float Ratio, coefficient_t Modulus){

	file_format format = DISTANCE_MATRIX;

	index_t dim_max = dimMax;
	value_t threshold = Threshold;
	float ratio = Ratio;
	coefficient_t modulus = Modulus;

	compressed_lower_distance_matrix dist = read_graph(graph, num_entries);
	...
	std::cout << "sparse distance matrix with " << dist.size() << " points and "
				...
				<< std::endl;
	*/
	float * resultado = ripser<sparse_distance_matrix>(sparse_distance_matrix(std::move(dist), threshold),dim_max, threshold, ratio, modulus).compute_barcodes();

	if(dim_max == 1 && *(resultado + 1) == 0) std::cout << "[" << *resultado << "]" << std::endl;
	else{
		std::cout << "[";
...
int TrivialHomology( double * graph, double num_entries, index_t dimMax, value_t Threshold, float Ratio, coefficient_t Modulus){
...
void writeGraph(double * entries, int numVertices){
	//Seed for random numbers generation
    srand((unsigned int)time(NULL));
    ...
    //File is opened and the random graph is written
    int c = 0;
    file.open("graph.txt");
    file << "\n";
    for(int i = 0 ; i < numVertices - 1 ; i++){
        for(int j = 0 ; j < i + 1 ; j++){
            randomDistance = *(entries + c);
            file << randomDistance;
            if(j < numVertices - 2){
...
//void print_graph_F_ts(unsigned short int G[MaxV][MaxV],int n,FILE *OF){
//	int i,j; 
//	for(i=0;i<n-1;i++){
//		for(j=i+1;j<n;j++){
//	       fprintf(OF,"%d",G[i][j]);
...
void MakeDirS(void){
	...
	F=fopen("Flag.dat","r");
	if(F==NULL){
	  printf("Problems with Flag.dat file, does not exist") ; getchar();	
	}	
	fscanf(F,"%d",&flag);fclose(F);
	
	if(flag==0){
		if(OS==0){
				system("mkdir ag");system("mkdir 1v");system("mkdir 2v");system("mkdir 3v");system("mkdir 4v");system("mkdir 5v");system("mkdir 6v");
				...
				system("mv 1v ag");system("mv 2v ag");system("mv 3v ag");system("mv 4v ag");system("mv 5v ag");system("mv 6v ag");
				system("mv 7v ag");system("mv 8v ag");system("mv 9v ag");system("mv 10v ag");system("mv 11v ag");			
		     }else{
				system("md ag");system("md 1v");system("md 2v");system("md 3v");system("md 4v");system("md 5v");system("md 6v");
				...
				system("move 1v ag");system("move 2v ag");system("move 3v ag");system("move 4v ag");system("move 5v ag");system("move 6v ag");
				system("move 7v ag");system("move 8v ag");system("move 9v ag");system("move 10v ag");system("move 11v ag");
     	}
	    flag=1;F=fopen("Flag.dat","w");fprintf(F,"%d",flag);fclose(F);
  }
}
    ...
    if(argc==1 || argc==3){
    	    ...
    	    if(V<11 ||(V==11)&&(E<=16 || E>=40)){
				...
				for(ii=1;ii<=Card;ii++){
						...
						for(j=1;j<V;j++){
							for(i=0;i<j;i++){
								*(graph+kk)=1-OnesZ[kk];//	G[i][j]=OnesZ[kk]; G[j][i]=G[i][j];
								kk++;
							}
						//	G[j-1][j-1]=0;
						}
		    	...
		    	printf("\n where M indicate what Million is  (See #cgVE.dat file)");
