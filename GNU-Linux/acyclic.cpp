/*
  This program runs over the connected graphs, in format ._g6.
  The output is the family of acyclic graphs (those graphs whose clique complex
  have trivial simplicial homology with coefficients in Z/2Z) up to 11 vertices.
  
  Use as:
  ./acyclic             : Interger number V and E will be requested after.
  ./acyclic V           : Graphs on V vertices and E=V-1,V,V+1,..., V(V-1)/2 will be processed.
  ./acyclic V E         : Graphs on V vertices and E edges will be processed.
  ./acyclic V E n1      : Graphs on V vertices, E edges and n1-th million will be processed.
  ./acyclic V E n1 n2   : Graphs on V vertices, E edges and n1_th to n1_th (millions) will be processed.
*/
...
void MakeDirS(void){
    ...
    if(F==NULL){
        printf("File FlagAcyclic.dat not found. Please check.") ; getchar();
    }
...
void Back(void){
    int i; 
    for(i=1;i<=11;i++){
        printf("%c",8);// Move 10 places left
    }
}
...
int main(int argc, char* argv[]) {
    ...
    if(argc==1 || argc==3){
        if(V<11 ||(V==11)&&(E<=16 || E>=40)){
            ...
            printf("\n%ld graphs with V=%d, E=%d, have trivial homology.\n",ngHT,V,E);
            fclose(OF_c);
        }else{
            printf("\nInstead of use ./acyclic 11");
            printf("\nuse ./acyclic 11 E");
            printf("\nwith E=1,2,...,16 and");
            printf("\nwith E=40,41,...,55.");
            printf("\nFor E=17,18,..39,");
    ...
    if(argc==2){
        if(V<11){
            ...
            for(E=V-1;E<=V*(V-1)/2;E++){
                ...
                fprintf(OF_c,"%ld",ngHT);
                printf("\n%ld graphs with V=%d and E=%d have trivial homology.\n",ngHT,V,E);
                fclose(OF_c);
            }
        }else{
            printf("\nInstead of use ./acyclic 11");
    ...
    if(argc==4 || argc==5){
        if( V==11 && E>=17 && E<=39){
            ...
            for(ene=n1;ene<=n2;ene++){
                ...
                fprintf(OF_c,"%ld",ngHT);
                printf("\n%ld graphs with V=%d, E=%d of %d million have trivial homology.\n",ngHT,V,E,ene);
                fclose(OF_c);
            }
        }else{
