...
#include<time.h>
/*
This program runs over simple graphs of https://users.cecs.anu.edu.au/~bdm/data/graphs.html
and save in several files depending of number of vertices and edges (v,e)  (to make easy parallel work).

Also eliminate N[n] of format .g6 of Mckey to reduce the size of stored files. The new format is called ._g6
...
void Back(void){
    int i; 
    for(i=1;i<=11;i++) printf("%c",8); // Move 10 places left
}
...
void MakeDirV(int v){// Create a directory for the vertex v
...
void MakeDirVE(int v, int e){ // Create a directory for the vertex v and edge e
    char Num[30],Comm1[50];
    strcpy(Comm1,"mkdir ");
    sprintf(Num,"%d",v); strcat(Comm1,Num); strcat(Comm1,"v");
...
void MoveToDir(int v){ // Move files of v vertices and all edges to the corresponding directory
...
void CopyFile(int v, int e, char NameOut[30], char NameOutM[30]){// Copy files
...
void MoveDirToDir(int v, int e){// Relocate directories
    char Num[30],Comm1[50],Name[30],NameT[30];
...
void GetA_M(int Binary[64][6], char Num[12], int G[MaxV][MaxV], int V){
    ...
    } G[V-1][V-1]=0; //Last element of diagonal
}
...
int IsConnected(int G[MaxV][MaxV], int V){
    ...
    //Search isolated vertex
    if(ExistV_isolated(G,V)==1) return 0;
    
    // Put marks to Vertex 0 and their neighborhoods
    Mark[0]=0;nM=1; for(j=1;j<V;j++) if(G[0][j]==1){ Mark[nM]=j;nM++;}  if(nM==V) return 1;
    
    // Mark any vertex no marked, neighbor of each marked vertex (except mark[0])
    for(k=1;k<nM;k++){
        // Search neighbors j of vertex=Mark[k]
        RespnM=nM;
        for(j=0;j<V;j++){
            if(G[Mark[k]][j]==1){ // find a neighbor j
                if(IsJreg(j,Mark,nM)==0){  // is not vertex j registered?
                    Mark[nM]=j; nM++;
                    if(nM==V) return 1;
                }
            }
...
void PrintGtoD(int k, int G[MaxV][MaxV], int V){
    ...
    for(i=0;i<V-1;i++){
        printf("%2d  ",i); for(j=0;j<=i;j++) printf("---"); for(j=i+1;j<V;j++) printf("%2d ",G[i][j]);
        printf("\n");
    } printf("\n");
}
...
// Special tratament for V=11
    ...
    printf("\n%0.0lf graphs processed per second. Press enter to exit.\n",Tg/time_spent);getchar();
    return 0;
}
